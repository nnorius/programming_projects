...
void main()
{
  ...
  gl_Position = vec4(scale*a_Position.x + xshift, scale*a_Position.y + yshift, a_Position.zw);
...
function draw(shiftValue1, shiftValue2, scaleValue)
{
  ...
  let index1 = gl.getUniformLocation(shader, "xshift");
  gl.uniform1f(index1, shiftValue1);

  let index2 = gl.getUniformLocation(shader, "yshift");
  gl.uniform1f(index2, shiftValue2);

  let index3 = gl.getUniformLocation(shader, "scale");
  gl.uniform1f(index3, scaleValue);
...
function main() {
  ...
  var angle = 0;
  var radius = 0.8;
  let xshift = radius * Math.cos(toRadians(angle));
  let yshift = radius * Math.sin(toRadians(angle));
  let scale = parseFloat(document.getElementById("scaleBox").value);

  // define an animation loop
  var animate = function() {

      draw(xshift, yshift, scale);


      xshift = radius * Math.cos(toRadians(angle));
      yshift = radius * Math.sin(toRadians(angle));
      angle += increment;
